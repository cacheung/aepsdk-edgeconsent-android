...
class ConsentExtension extends Extension {
     ...
     */
    protected ConsentExtension(
            final ExtensionApi extensionApi, final NamedCollection namedCollection) {
     ...
     */
    protected ConsentExtension(
            final ExtensionApi extensionApi, final ConsentManager consentManager) {
        super(extensionApi);
    ...
    protected void onRegistered() {
        ...
        getApi().registerEventListener(
                        EventType.CONSENT, EventSource.UPDATE_CONSENT, this::handleConsentUpdate);
        getApi().registerEventListener(
                        EventType.CONSENT, EventSource.REQUEST_CONTENT, this::handleRequestContent);
        getApi().registerEventListener(
    ...
    void handleEdgeConsentPreferenceHandle(@NonNull final Event event) {
        ...
        final List<Map<String, Object>> payload =
                DataReader.optTypedListOfMap(
                        Object.class, eventData, ConsentConstants.EventDataKey.PAYLOAD, null);

        if (payload == null || payload.isEmpty()) {
            Log.debug(
                    LOG_TAG,
                    LOG_SOURCE,
                    "Ignoring the consent:preferences handle event from Edge Network, empty/missing"
                            + " payload.");
            return;
        }
        ...
        if (newConsents.isEmpty()) {
            Log.debug(
                    LOG_TAG,
                    LOG_SOURCE,
                    "Ignoring the consent:preferences handle event from Edge Network, no valid"
                            + " consent data found.");
            return;
        }
        ...
        if (newConsents.getTimestamp() == null
                || newConsents.getTimestamp().equals(currentConsent.getTimestamp())) {
    ...
    private void dispatchEdgeConsentUpdateEvent(final Consents consents) {
        ...
        final Event edgeConsentUpdateEvent =
                new Event.Builder(
