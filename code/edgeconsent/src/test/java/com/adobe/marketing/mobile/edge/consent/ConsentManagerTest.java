...
public class ConsentManagerTest {
    ...
    public void test_Constructor_LoadsFromPersistence() {
        // setup
        final String updatedConsentsJSON =
                CreateConsentsXDMJSONString("y", null, SAMPLE_METADATA_TIMESTAMP);
        Mockito.when(
    ...
    public void test_MergeAndPersist() {
        ...
        final String persistedJSON =
                CreateConsentsXDMJSONString("y", "n", "vi", SAMPLE_METADATA_TIMESTAMP);
        Mockito.when(
                        mockNamedCollection.getString(
                                ConsentConstants.DataStoreKey.CONSENT_PREFERENCES, null))
        ...
        consentManager =
                new ConsentManager(
                        mockNamedCollection); // consentManager now loads the persisted data

        // test
        Consents newConsent =
                new Consents(CreateConsentXDMMap("n", null, "pi", SAMPLE_METADATA_TIMESTAMP_OTHER));
        ...
        assertEquals(
                "n",
                readCollectConsent(
                        mergedConsent)); // assert CollectConsent value has changed on merge
        assertEquals(
                "n",
                readAdIdConsent(
                        mergedConsent)); // assert adIdConsent value has not changed on merge
        assertEquals(
                "pi",
                readPersonalizeConsent(
                        mergedConsent)); // assert PersonalizeConsent value has changed on merge
        assertEquals(
        ...
        verify(mockNamedCollection, times(1))
                .setString(
                        ...
                        CreateConsentsXDMJSONString(
                                "n", "n", "pi", SAMPLE_METADATA_TIMESTAMP_OTHER));
    }
    ...
    public void test_MergeAndPersist_nullConsent() {
        ...
        assertEquals(
                "y",
                readCollectConsent(
                        mergedConsent)); // assert CollectConsent value has not changed on merge
        assertEquals(
                "n",
                readAdIdConsent(
                        mergedConsent)); // assert adIdConsent value has not changed on merge
        assertEquals(
                ...
                ConsentTestUtil.readTimestamp(
                        mergedConsent)); // assert time has not changed on merge
    ...
    public void test_MergeAndPersist_whenExistingConsentsNull_AndNewConsentValid() {
        ...
        consentManager =
                new ConsentManager(
                        mockNamedCollection); // consentManager now loads nothing from persisted
        // data
    ...
    public void test_MergeAndPersist_whenNullNamedCollection() {
        ...
        assertEquals(
                "n", readCollectConsent(mergedConsent)); // assert CollectConsent value is merged
        // no exception is expected when attempting to read current consents or write new consents
    }
    ...
    public void test_MergeAndPersist_whenExistingAndNewConsentEmpty() {
        ...
        verify(mockNamedCollection, times(1))
                .remove(ConsentConstants.DataStoreKey.CONSENT_PREFERENCES);
    }
    ...
    public void test_updateDefaultConsents() {
        ...
        boolean isCurrentConsentChanged =
                consentManager.updateDefaultConsents(new Consents(CreateConsentXDMMap("n")));
    ...
    public void test_updateDefaultConsents_whenCurrentConsentAlreadySet_ShouldNotUpdate() {
        ...
        consentManager.mergeAndPersist(
                new Consents(CreateConsentXDMMap("y", "n", "vi", SAMPLE_METADATA_TIMESTAMP)));

        // test
        // update default consent with Collect NO adID NO
        boolean isCurrentConsentChanged =
                consentManager.updateDefaultConsents(new Consents(CreateConsentXDMMap("n", "n")));
