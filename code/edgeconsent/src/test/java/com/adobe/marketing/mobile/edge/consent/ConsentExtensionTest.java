...
public class ConsentExtensionTest {
    ...
    public void test_listenerRegistration() {
        ...
        ArgumentCaptor<ExtensionEventListener> extensionEventListenerArgumentCaptor =
        ...
        List<ExtensionEventListener> extensionEventListenerList =
                extensionEventListenerArgumentCaptor.getAllValues();
    ...
    public void test_OnBootUp_SharesXDMSharedState() {
        ...
        verify(mockExtensionApi, times(1))
                .createXDMSharedState(
                        sharedStateCaptor.capture(), sharedStateEventCaptor.capture());
        ...
        assertEquals(
                "y",
                ((Map) ((Map) consentResponseEvent.getEventData().get("consents")).get("collect"))
                        .get("val"));
    }
    ...
    public void test_getName() {
        ...
        assertEquals(
                ...
                ConsentConstants.EXTENSION_NAME,
                moduleName);
    }
    ...
    public void test_handleConfigurationResponse() throws Exception {
        ...
        verify(mockExtensionApi, times(1))
                .createXDMSharedState(sharedStateCaptor.capture(), eq(configEvent));
    ...
    public void test_handleConfigurationResponse_RemoveDefault() throws Exception {
        ...
        verify(mockExtensionApi, times(1))
                .createXDMSharedState(
                        eq(ConsentTestUtil.emptyConsentXDMMap()), eq(emptyConfigEvent));
    ...
    public void test_handleConsentUpdate() {
        ...
        assertEquals(
                "n",
                ((Map) ((Map) consentResponseEvent.getEventData().get("consents")).get("adID"))
                        .get("val"));
        assertNotNull(
                ((Map) ((Map) consentResponseEvent.getEventData().get("consents")).get("metadata"))
                        .get("time"));

        // verify edge consent event dispatched
        Event edgeConsentUpdateEvent = eventCaptor.getAllValues().get(1);
        assertEquals(
                edgeConsentUpdateEvent.getName(), ConsentConstants.EventNames.EDGE_CONSENT_UPDATE);
        ...
        assertEquals(
                "y",
                ((Map) ((Map) edgeConsentUpdateEvent.getEventData().get("consents")).get("collect"))
                        .get("val"));
        assertEquals(
                "n",
                ((Map) ((Map) edgeConsentUpdateEvent.getEventData().get("consents")).get("adID"))
                        .get("val"));
        assertNotNull(
                ((Map)
                                ((Map) edgeConsentUpdateEvent.getEventData().get("consents"))
                                        .get("metadata"))
                        .get("time"));
    }
    ...
    public void test_handleConsentUpdate_MergesWithExistingConsents() {
        ...
        extension.handleConsentUpdate(
                consentUpdateEvent); // send update event which overrides collect consent to YES
        ...
        verify(mockExtensionApi, times(1))
                .createXDMSharedState(sharedStateCaptor.capture(), eq(consentUpdateEvent));
        Map<String, Object> sharedState = sharedStateCaptor.getValue();
        assertEquals("y", ((Map) ((Map) sharedState.get("consents")).get("collect")).get("val"));
    ...
    public void test_handleConsentUpdate_NullEventData() {
        // setup
        Event event =
                new Event.Builder("Consent Response", EventType.CONSENT, EventSource.UPDATE_CONSENT)
                        .setEventData(null)
                        .build();
        ArgumentCaptor<Event> eventCaptor = ArgumentCaptor.forClass(Event.class);
    ...
    public void test_handleEdgeConsentPreferenceHandle() throws Exception {
        ...
        // verify XDM shared state is set
        verify(mockExtensionApi, times(1))
                .createXDMSharedState(sharedStateCaptor.capture(), eq(event));
        Map<String, Object> sharedState = sharedStateCaptor.getValue();
        assertEquals("y", ((Map) ((Map) sharedState.get("consents")).get("collect")).get("val"));
        assertEquals("n", ((Map) ((Map) sharedState.get("consents")).get("adID")).get("val"));
        assertEquals(
                "y",
                ((Map)
                                ((Map) ((Map) sharedState.get("consents")).get("personalize"))
                                        .get("content"))
                        .get("val"));
        assertNotNull(((Map) ((Map) sharedState.get("consents")).get("metadata")).get("time"));
    ...
    @Test
    public void test_handleEdgeConsentPreferenceHandle_MergesWithExistingConsents()
            throws Exception {
        // setup
        setupExistingConsents(CreateConsentsXDMJSONString("n", "n"));
        Event event =
    ...
    public void test_handleEdgeConsentPreferenceHandle_SameConsentAndTimeStamp() throws Exception {
        ...
        Event event =
                buildEdgeConsentPreferenceEventWithConsents(
                        CreateConsentXDMMap("y", "y", "sometime"));
    ...
    @Test
    public void test_handleEdgeConsentPreferenceHandle_SameConsentAndNoTimeStamp()
            throws Exception {
        // setup
        setupExistingConsents(CreateConsentsXDMJSONString("y", "y", "sometime"));
    ...
    @Test
    public void test_handleEdgeConsentPreferenceHandle_SameConsentAndDifferentTimeStamp()
            throws Exception {
        // setup
        setupExistingConsents(CreateConsentsXDMJSONString("y", "y", "sometime"));
        Event event =
                buildEdgeConsentPreferenceEventWithConsents(
                        CreateConsentXDMMap("y", "y", "different"));
        final ArgumentCaptor<Map> sharedStateCaptor = ArgumentCaptor.forClass(Map.class);
        final ArgumentCaptor<Event> eventCaptor = ArgumentCaptor.forClass(Event.class);
    ...
    @Test
    public void
            test_handleEdgeConsentPreferenceHandle_SameConsentAndDifferentMetadataWithNoTimeStamp()
                    throws Exception {
        // setup
        setupExistingConsents(CreateConsentsXDMJSONString("y", "y", "sometime"));
        Event event =
    ...
    private void setupExistingConsents(final String jsonString) {
        Mockito.when(
                        mockNamedCollection.getString(
                                ConsentConstants.DataStoreKey.CONSENT_PREFERENCES, null))
                .thenReturn(jsonString);
        extension = new ConsentExtension(mockExtensionApi, mockNamedCollection);
    }

    private Event buildConsentUpdateEvent(
            final String collectConsentString, final String adIdConsentString) {
        Map<String, Object> eventData =
                CreateConsentXDMMap(collectConsentString, adIdConsentString);
    ...
    private Event buildEdgeConsentPreferenceEvent(final String jsonString) throws JSONException {
        ...
        return new Event.Builder(
                        "Edge Consent Preference", EventType.EDGE, EventSource.CONSENT_PREFERENCE)
                .setEventData(eventData)
                .build();
    }
    ...
    private Event buildConfigurationResponseEvent(final String jsonString) throws JSONException {
        ...
        Map<String, Object> configEventData =
